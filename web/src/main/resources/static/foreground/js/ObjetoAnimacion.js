...
var FuncionesTiempo = {
    Linear: function (Tiempo) {
        return Tiempo;
    },
    SinIn: function (Tiempo) {
        return 1 - Math.cos(Tiempo * Math.PI / 2);
    },
    SinOut: function (Tiempo) {
        return Math.sin(Tiempo * Math.PI / 2);
    },
    SinInOut: function (Tiempo) {
        return -0.5 * (Math.cos(Math.PI * Tiempo) - 1);
    },
...
var ObjetoAnimacion = function () {
    ...
    this.Actualizar = function (nTick) {
        if (this.EnPausa === true) {
            return;
        }
//        var Tick;
        if (typeof(nTick) === 'undefined') {
            this.Tick = Date.now();
        }
        else {
            this.Tick = nTick;
...
//        this._Invertido          = false;        // Invertir animación
    ...
    this._Opciones = {
        Repetir: 0, Invertir: false, FuncionActualizar: function (Valores) {
        }, FuncionTerminado: function () {
        }
    };
    if (typeof (Opciones) !== 'undefined') {
        if (typeof Opciones.Repetir !== "undefined") {
            this._Opciones.Repetir = Opciones.Repetir;
        }
        if (typeof Opciones.Invertir !== "undefined") {
            this._Opciones.Invertir = Opciones.Invertir;
        }
    ...
    for (var Paso in this._Pasos) {
        if (typeof this._Pasos[Paso].Tiempo === "undefined") {
            this._Pasos[Paso].Tiempo = 1;
        }
        if (typeof this._Pasos[Paso].Retraso === "undefined") {
            this._Pasos[Paso].Retraso = 0;
        }
        if (typeof this._Pasos[Paso].FuncionTiempo === "undefined") {
            this._Pasos[Paso].FuncionTiempo = FuncionesTiempo.Linear;
        }
    }
