layui.define(['element', 'laypage', 'layer', 'form', 'pagesize'], function (exports) {
    ...
    function initilData(currentIndex, pageSize) {
        ...
        setTimeout(function () {
            layer.close(index);
            $.ajax({
                url: "/admin/moodManage/list/" + currentIndex,
                type: "GET",
                data: {
                    pageSize: pageSize
                },
                success: function (moodList) {
                    ...
                    html += '<colgroup><col width="30"><col width="90"><col width="200"><col width="90"><col width="75"><col width="50"><col width="40"><col width="40"></colgroup>';
                    html += '<thead><tr><th>编号</th><th>标题</th><th>内容</th><th>时间</th><th>照片墙</th><th>状态</th><th colspan="2">操作</th></tr></thead>';
                    html += '<tbody>';
                    for (var i in moodList) {
                        var item = moodList[i];
                        html += '<tr>';
                        html += '<td>' + item.id + '</td>';
                        ...
                        html += '<td>'+'<img src="'+ item.pictureUrl +'" width="175px"/>'+ '</td>';
                        if (item.is_show) {
    ...
    var datalist = {
        deleteData: function (id) {
            layer.confirm('同时会删除对应照片墙的照片，确定删除？', {
                btn: ['确定', '取消'] //按钮
            }, function () {
                $.ajax({
                    url: '/admin/mood/' + id,
                    ...
                    success: function (result) {
                        ...
                        if (result.success) {
                            layer.alert('删除成功!', {
                                ...
                                window.location.href = "/admin/moodManage"
                            });
                        } else {
        ...
        editData: function (id) {
            parent.switchTab(parent.$, parent.element, '修改博客', '/admin/writeMood?id=' + id, 'Mood' + id);
        }
    };
